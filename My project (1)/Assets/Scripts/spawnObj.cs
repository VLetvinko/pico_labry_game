...
namespace Generator
{
    public class spawnObj : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            mazeArray = myLevelList.level[currentLevel].levelArray;
        ...
        private void generateHorizontalWall()
        {
            ...
            for (int i = 0; i < mazeArray.GetLength(0); i += 2)
            {
                int zHorizontalWall = 15;
                for (int j = 1; j < mazeArray[i].Length; j += 2)
                {
        ...
        private void generateVerticalWall()
        {
            ...
            {
                int xVerticalWall = 6;
                for (int j = 0; j < mazeArray[i].Length; j += 2)
                {
                    switch (mazeArray[i][j])
                    {
                        case '#':
        ...
        private void generateGroundAndMore()
        {
            ...
            var select = spawnPlayer.transform.GetChild(PlayerPrefs.GetInt("SelectPlayer")).gameObject;
            ...
            for (int i = 1; i < mazeArray.GetLength(0); i += 2)
            {
                ...
                for (int j = 1; j < mazeArray[i].Length; j += 2)
                {
                    ...
                    switch (mazeArray[i][j])
                    {
                        ...
                        case 's':
                            Instantiate(Grounds[0], new Vector3(xGround, 0, zGround), Quaternion.identity);
                            select.transform.position = new Vector3(xGround, 10, zGround);
                            break;
                        case 'f':
        ...
        {
            float getSizeRow = (mazeArray.GetLength(0) - 1) / 2;
            float getSizeCol = (mazeArray[0].Length - 1) / 2;
